...
module PAYJPv2
  class ApiClient
    ...
    def initialize(config = Configuration.default)
      @config = config
      @user_agent = "payjp/payjpv2 RubyBindings/#{VERSION}"
      @client_user_agent = build_client_user_agent
      @default_headers = {
        'Content-Type' => 'application/json',
        'User-Agent' => @user_agent,
        'X-Payjp-Client-User-Agent' => @client_user_agent
      }
    end
    ...
    def download_file(request)
      ...
      request.on_complete do
        if !tempfile
          raise ApiError.new("Failed to create the tempfile based on the HTTP response from the server: #{request.inspect}")
        end
        tempfile.close
    ...
    # Build the client user agent information for X-Payjp-Client-User-Agent header
    #
    # @return [String] JSON string containing client information
