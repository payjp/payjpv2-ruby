...
module PAYJPv2
  class BalanceResponse
    ...
    # Balanceの状態  | 指定できる値 | |:---| | **collecting**: 集計中 | | **transfer**: 入金 | | **claim**: 請求 |
    attr_accessor :state

    # 関連付けられているStatementオブジェクトのリスト
    attr_accessor :statements

    # このBalanceの清算が終了していればtrue  state=transferであれば加盟店口座への入金作業完了、state=claimであればPAY.JPで請求額の振込が確認できたことを表します。
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PAYJPv2::BalanceResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PAYJPv2::BalanceResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [object, id, livemode, created_at, updated_at, state, statements, closed, closed_date, due_date, net, bank_info].hash
    end
