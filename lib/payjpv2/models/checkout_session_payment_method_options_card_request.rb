  ...
  class CheckoutSessionPaymentMethodOptionsCardRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        raise ArgumentError, "The input argument (attributes) must be a hash in `PAYJPv2::CheckoutSessionPaymentMethodOptionsCardRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          raise ArgumentError, "`#{k}` is not a valid attribute in `PAYJPv2::CheckoutSessionPaymentMethodOptionsCardRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def request_three_d_secure=(request_three_d_secure)
      ...
      unless validator.valid?(request_three_d_secure)
        raise ArgumentError, "invalid value for \"request_three_d_secure\", must be one of #{validator.allowable_values}."
      end
      @request_three_d_secure = request_three_d_secure
    end
    ...
    def hash
      [request_extended_authorization, request_three_d_secure].hash
    end
