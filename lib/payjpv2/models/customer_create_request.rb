  ...
  class CustomerCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        raise ArgumentError, "The input argument (attributes) must be a hash in `PAYJPv2::CustomerCreateRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          raise ArgumentError, "`#{k}` is not a valid attribute in `PAYJPv2::CustomerCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def id=(id)
      ...
      if id.to_s.length > 100
        raise ArgumentError, 'invalid value for "id", the character length must be smaller than or equal to 100.'
      end

      pattern = Regexp.new(/^[-_0-9A-Za-z]+$/)
      if id !~ pattern
        raise ArgumentError, "invalid value for \"id\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [id, payment_method_id, email, description, metadata].hash
    end
