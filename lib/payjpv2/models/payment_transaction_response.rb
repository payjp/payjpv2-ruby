...
module PAYJPv2
  class PaymentTransactionResponse
    ...
    # PaymentTransaction の種類  | 値 | |:---| | **payment**: 支払い | | **refund**: 返金 | | **chargeback**: チャージバック | | **chargeback_cancel**: チャージバックのキャンセル | 
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        raise ArgumentError, "The input argument (attributes) must be a hash in `PAYJPv2::PaymentTransactionResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          raise ArgumentError, "`#{k}` is not a valid attribute in `PAYJPv2::PaymentTransactionResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def resource_id=(resource_id)
      if resource_id.nil?
        raise ArgumentError, 'resource_id cannot be nil'
      end
    ...
    def fee_rate=(fee_rate)
      if fee_rate.nil?
        raise ArgumentError, 'fee_rate cannot be nil'
      end

      pattern = Regexp.new(/^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$/)
      if fee_rate !~ pattern
        raise ArgumentError, "invalid value for \"fee_rate\", must conform to the pattern #{pattern}."
      end
    ...
    def fee=(fee)
      if fee.nil?
        raise ArgumentError, 'fee cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] term_id Value to be assigned
    def term_id=(term_id)
      if term_id.nil?
        raise ArgumentError, 'term_id cannot be nil'
      end
    ...
    def hash
      [object, id, livemode, resource_id, amount, currency, fee_rate, fee, type, payment_method_type, term_id, created_at, updated_at].hash
    end
